    ...
    <AdminLayout breadcrumb="Overview / Datasets" title="Datasets">
      ...
      <section className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <div>
            <h2 className="text-admin-fg text-lg font-bold mb-0.5">Content datasets</h2>
        ...
        {showCreate && (
          ...
          >
            <label className="flex flex-col">
              <span className="text-admin-muted text-sm font-semibold mb-1.5">Name *</span>
              <input
                type="text"
                value={newName}
                onChange={(e) => setNewName(e.target.value)}
                placeholder="e.g. Bible (NWT)"
                className="rounded-lg border border-admin-border bg-admin-muted-surface text-admin-fg focus:ring-primary focus:border-primary h-11 px-4"
                required
              />
            </label>
            <label className="flex flex-col">
              <span className="text-admin-muted text-sm font-semibold mb-1.5">Source / attribution</span>
              <input
                type="text"
                value={newSource}
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button
                ...
                className="px-4 py-2 rounded-lg bg-admin-muted-surface text-admin-fg font-medium hover:bg-admin-muted-surface"
              >
                Cancel
              </button>
              <button
        ...
        <div className="bg-admin-panel rounded-md border border-admin-border shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-admin-border">
                ...
                {datasets.map((dataset) => {
                  ...
                  return (
                    <tr key={dataset.id} className="hover:bg-admin-muted-surface/50">
                      <td className="px-4 py-4 font-medium text-admin-fg whitespace-nowrap">
                        {dataset.name}
                        {dataset.is_official && (
                      ...
                      <td className="px-4 py-4 text-admin-muted">{dataset.source || '-'}</td>
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold transition-colors disabled:opacity-50 ${
                            dataset.is_enabled
                              ...
                              : 'bg-admin-muted-surface text-admin-muted hover:bg-admin-muted-surface'
                          }`}
                          title={dataset.is_enabled ? 'Click to disable' : 'Click to enable'}
                        >
                          <span className={`size-2 rounded-full ${dataset.is_enabled ? 'bg-green-500' : 'bg-admin-muted'}`} />
                      ...
                      <td className="px-4 py-4">
                        <label className="inline-flex items-center gap-2 cursor-pointer">
                          ...
                          />
                          <span className="text-admin-muted text-xs">{dataset.is_default ? 'Default' : 'Set default'}</span>
                        </label>
                      </td>
                      <td className="px-4 py-4 text-right">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-admin-border text-admin-fg hover:bg-admin-muted-surface font-medium text-sm"
                        >
          ...
          <div className="px-4 py-3 border-t border-admin-border flex items-center justify-between bg-admin-muted-surface/50 text-admin-muted text-sm">
