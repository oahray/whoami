...
export default function AdminActiveDatasetBar({ hidden }: AdminActiveDatasetBarProps) {
  ...
  if (loading && !selectedDataset) {
    return (
      <div className="border-b border-admin-border bg-admin-panel px-4 py-2 md:px-6">
        <p className="text-sm text-admin-muted">Loading datasets…</p>
      </div>
    )
  }
    ...
    return (
      <div className="border-b border-primary/20 bg-primary/5 px-4 py-2 md:px-6">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <p className="min-w-0 flex-1 text-sm text-admin-muted truncate">
            <span className="font-semibold text-admin-muted">Active:</span>{' '}
            <span className="font-bold text-admin-fg">{selectedDataset.name}</span>
          </p>
          ...
          <button
            ...
            className="flex size-8 shrink-0 items-center justify-center rounded-lg text-admin-muted hover:bg-admin-panel/80 transition-colors"
            aria-expanded={false}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-lg font-bold text-admin-fg truncate">{selectedDataset.name}</p>
            {selectedDataset.source && (
              <p className="text-sm text-admin-muted truncate">{selectedDataset.source}</p>
            )}
          ...
          <button
            type="button"
            onClick={toggleCollapsed}
            className="flex size-8 shrink-0 items-center justify-center rounded-lg text-admin-muted hover:bg-admin-panel/80 transition-colors sm:mt-0.5"
            aria-expanded={true}
            aria-controls={panelId}
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0 sm:pt-1">
          {enabledDatasets.length > 1 ? (
            <label className="flex flex-col gap-1 sm:min-w-[12rem]">
              <span className="text-xs font-semibold text-admin-muted sm:sr-only">Switch dataset</span>
              <select
                value={selectedDatasetId ?? ''}
                onChange={(e) => setSelectedDatasetId(e.target.value || null)}
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-admin-panel border border-admin-border px-3 py-1.5 text-xs font-semibold text-admin-muted">
          ...
          <Link
            to="/datasets"
            className="inline-flex items-center justify-center gap-1 rounded-lg border border-admin-border bg-admin-panel px-3 py-2 text-sm font-semibold text-admin-fg hover:bg-admin-muted-surface"
          >
            All datasets
